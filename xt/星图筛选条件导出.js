...
(function () {
  ...
  const request = function(mothed, url, param){   //网络请求
  ...
  function inlyModelComponent_login(){
      ...
      <div id="inly_box" style="position:absolute;top:50%;left: 50%;transform: translate(-50%, -100%);height: 200px;width: 350px;background-color: #fff;border-radius: 10px;display: flex;justify-content: center;align-items: center;">
        ...
        <div style="position: absolute;top: 0px;right: 10px; z-index: -1; opacity: 0.5;width: 160px;">
          ...
          <form action="" method="post" id="form1" >
            <p style="margin-bottom: 10px;font-size: 14px;">账号: <input type="text" name="name" style="background-color: rgba(255,255,255,0.2);border:0px;border-bottom: 1px solid #999;"/></p>
            <p style="margin-bottom: 20px;font-size: 14px;">密码: <input type="password" name="password" style="background-color: rgba(255,255,255,0.2);border:0px;border-bottom: 1px solid #999;"/></p>
    ...
    //{account:name_1,password:name_2}
    //登录按钮事件
    $("#inly_submit").click(function(e){ 
      var name_1=$("input[name='name']").val();   //获取
      var name_2=$("input[name='password']").val();   //获取
      console.log(name_1,name_2)
      let data = new FormData()
      data.append('account',name_1)
      data.append('password',name_2)
      request('post','http://192.168.12.145:8080/user_app/login/',data).then((res) => {
...
//   button.style.backgroundSize = "98px"
  ...
  function listen() {
    ...
    function replaceFn(obj) {
      // console.log('监听', this._url)
      apiLists.push(this._url);
    }
  }
  listen()
  ...
  //字符串截取
  function stringSlice(text,e){
    let reg = new RegExp(',','g')
    let arr = [...text]
    let arr2 = []
    arr.map((v,i)=>{if (i>=e) arr2.push(v)})
    return arr2.toString().replace(reg,'')
  }

  //query参数获取
  function getQueryVariable(url, variable) {
    var query = url.slice(28);
    // let query =  stringSlice(url,28)
  ...
  async function getQueryString() {
    if (!apiLists.length) {
      loadingMsg2.error('参数获取失败，请刷新页面后重试')
      return
    }
